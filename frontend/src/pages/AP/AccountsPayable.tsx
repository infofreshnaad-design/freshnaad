    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="bg-slate-900 border-b border-slate-800 p-8 pt-12 relative overflow-hidden">
         <div className="absolute top-0 right-0 p-12 opacity-10 rotate-12">
             <Wallet size={180} className="text-blue-400" />
         </div>
         ...
         <div className="max-w-7xl mx-auto relative z-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[2.5rem] flex items-center gap-6 min-w-[320px]">
                   <div className="w-14 h-14 bg-blue-500/20 rounded-2xl flex items-center justify-center text-blue-400 shadow-xl shadow-blue-500/10">
                      <Wallet size={28} />
                   </div>
                   <div>
      ...
      <div className="max-w-7xl mx-auto p-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div className="relative flex-1 max-w-md group">
                ...
                <input 
                    ...
                    className="w-full bg-white border-2 border-slate-100 rounded-2xl py-4 pl-12 pr-4 font-bold text-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all outline-none"
                />
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading ? (
                ...
                <div 
                    key={party.id} 
                    className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-100 hover:border-blue-500 hover:shadow-2xl hover:shadow-blue-500/5 transition-all group cursor-pointer relative overflow-hidden"
                ...
                >
                    ...
                    <div className="absolute top-0 right-0 mt-8 mr-8">
                        ...
                        {party.status === 'ADVANCE' && <div className="bg-blue-50 text-blue-500 p-2 rounded-xl"><Clock size={20} /></div>}
                        {party.status === 'CLEARED' && <div className="bg-emerald-50 text-emerald-500 p-2 rounded-xl"><CheckCircle2 size={20} /></div>}
                    </div>

                    <div className="flex items-center gap-4 mb-6">
                        <div className="w-14 h-14 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center font-black group-hover:bg-blue-500 group-hover:text-white transition-colors">
                            <Truck size={24} />
                        </div>
                        <div>
                            <h3 className="text-xl font-black text-slate-800 leading-tight group-hover:text-blue-600 transition-colors text-nowrap truncate max-w-[180px]">{party.name}</h3>
                    ...
                    <div className="flex items-end justify-between border-t border-slate-50 pt-6">
                        <div>
                           ...
                           <p className={`text-2xl font-black ${party.balance > 0 ? 'text-red-500' : (party.balance < 0 ? 'text-blue-500' : 'text-emerald-500')}`}>
                              ₹{Math.abs(party.balance).toLocaleString('en-IN')}
                              {party.balance < 0 && <span className="text-[10px] ml-1 uppercase">Cr</span>}
                           </p>
                        </div>
                        <div className="w-10 h-10 bg-slate-50 rounded-full flex items-center justify-center text-slate-300 group-hover:bg-blue-500 group-hover:text-white transition-all">
